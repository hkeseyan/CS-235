function Leaf_NN_zeroone 
TRAIN = load('leaf.csv'); 

TRAIN = TRAIN(randperm(size(TRAIN,1)),:);
TRAIN(:,2)=[];                            % column 2 not useful
TEST = TRAIN(1:170,:);                 % pull out half the data set since 2-fold cross-validation
TRAIN = TRAIN(171:end,:);
...
for i = 1 : size(TRAIN,2)                       % zero-one normalization
        TRAIN(:,i) = TRAIN(:,i)-min(TRAIN(:,i));    % subtract each column by the smallest value in the column
        TRAIN(:,i) = TRAIN(:,i)/max(TRAIN(:,i));    % divide each column by the largest value in the column
end;   
