...
function banknote_LOOCV %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% (C) Eamonn Keogh %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
TRAIN = load('data_banknote_authentication.txt'); % Only these two lines need to be changed to test a different dataset. %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    correct = 0; 		   % Initialize the number we got correct
for j = 1 : size(TRAIN,1)-1
    TRAIN = load('data_banknote_authentication.txt');
    TEST = TRAIN(j,:);                  % identify the test example
    TRAIN(j,:) = [];                    % remove test example from training set

    % note that class labels are in column 5 for this data set
    TRAIN_class_labels = TRAIN(:,5);     % Pull out the class labels.
    TRAIN(:,5) = [];                    % Remove class labels from training set.
    TEST_class_labels = TEST(:,5);       % Pull out the class labels.
    TEST(:,5) = [];                      % Remove class labels from testing set.

    for i = 1 : size(TRAIN,2)                       % zero-one normalization
        for k = 1 : size(TRAIN,1)
            TRAIN(k,i) = TRAIN(k,i)-min(TRAIN(:,i));    % subtract each column by the smallest value in the column
            TRAIN(k,i) = TRAIN(k,i)/max(TRAIN(:,i));    % divide each column by the largest value in the column
        end
    end        
